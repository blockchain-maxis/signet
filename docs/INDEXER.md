...
| 2 | **deployment** | Every tick | Horizon `/accounts/{pubkey}/operations` for every `Wallet` row (200 most recent, desc), plus a transaction fetch per contract-creation op | `Contract`, clears `Wallet.indexRequestedAt` |
