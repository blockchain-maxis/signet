...
| `SIGNET_API_URL` | cli | Optional | `http://localhost:3000` | Base URL the `signet link` command (packages/cli) talks to when creating a pairing and polling for browser approval. Defaults to the local web app, which is also what `pnpm --filter @signet/web dev` serves. See [`CLI.md`](./CLI.md). |
